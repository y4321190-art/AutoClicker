...
public class MainActivity extends AppCompatActivity {
    ...
    private void pick(int idx){
        ...
        ((FrameLayout)overlayView).setBackgroundColor(0x55000033);
        TextView tv=new TextView(this);tv.setText("Nokta "+(idx+1)+" için dokun");
        tv.setTextColor(0xFFFFFFFF);tv.setTextSize(22);
        ((FrameLayout)overlayView).addView(tv,new FrameLayout.LayoutParams(-2,-2,Gravity.CENTER));
    ...
    private void togglePoint(int idx){
        ...
        btnToggle[idx].setText(active[idx]?"Kapat":"Aç");
    ...
    private void toggleAll(){
        if(ClickService.isRunning){
            ...
            ClickService.isRunning=false;btnGlobal.setText("▶ Başlat");
            for(int i=0;i<2;i++){active[i]=false;btnToggle[i].setText("Aç");}
        }else{
            ...
            try{
                ...
                if(arr.length()==0){Toast.makeText(this,"Önce nokta seç",Toast.LENGTH_SHORT).show();return;}
